...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t({ en: 'Invalid credentials', fa: 'اطلاعات ورود نامعتبر است' }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t({ en: 'Failed to sign in with Google', fa: 'ورود با گوگل ناموفق بود' }));
    } finally {
      setLoading(false);
    }
  };
...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t({ en: 'Failed to create account', fa: 'ایجاد حساب ناموفق بود' }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t({ en: 'Failed to sign up with Google', fa: 'ثبت‌نام با گوگل ناموفق بود' }));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 md:p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={t({ en: 'Min. 6 characters', fa: 'حداقل ۶ کاراکتر' })}
                  className="w-full pl-10 pr-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none" disabled={loading} />
              </div>
            </div>
            <button type="submit" className="w-full py-3 bg-orange-500 text-white font-bold rounded-lg hover:bg-orange-600 transition-colors disabled:opacity-50" disabled={loading}>
...
export function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t({ en: 'Failed to send reset email', fa: 'ارسال ایمیل بازیابی ناموفق بود' }));
    } finally {
      setLoading(false);
    }
  };
